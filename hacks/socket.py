...
async def connect(sid, environ):
    if (
        ...
        logger.warning(
            """
            You need to configure at least one of
            on_chat_start, on_message or on_audio_chunk callback
            """
        )
        return False
    user = None
...
async def process_message(session: WebsocketSession, payload: MessagePayload):
    ...
    try:
        ...
        if config.code.on_message:
            # Sleep 1ms to make sure any children step
            # starts after the message step start
            time.sleep(0.001)
